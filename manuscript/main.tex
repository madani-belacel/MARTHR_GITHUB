...
\caption{MARTHR Protocol Architecture. Modular design with context fusion, trust management, score computation, and rank calculation components.}
...
\caption{Context-Dependent Weight Adaptation. Weights shift dynamically across safety, threat, and energy domains to emphasize the most critical objective.}
\label{fig:weights}
\end{figure}
...
\caption{DODAG with Trust-Aware Coloring. Higher trust values are shown in darker shades, indicating preferred routing paths.}
\label{fig:dodag}
\end{figure}
...
\caption{MARTHR vs MRHOF Baseline. Comparison across lossless, lossy, and attack scenarios. MARTHR achieves higher MCS and trust scores while maintaining competitive latency.}
\label{fig:baseline}
\end{figure}
...
\caption{MCS Across Scenarios. Mean Multi-Criteria Score from the current reproduction workflow. Values reflect present pipeline outputs.}
\label{fig:mcs}
\end{figure}
...
\caption{Energy and MCS Summary. MARTHR maintains high energy efficiency while achieving competitive MCS scores across representative scenarios.}
\label{fig:summary}
\end{figure}
...
\caption{Trust Score Distributions. Values across scenarios from the current simulation outputs, reflecting the present prototype pipeline.}
\label{fig:trust}
\end{figure}
...
\caption{Energy Consumption Across Scenarios. Illustrates the present generated outputs from the reproduction workflow.}
\label{fig:energy}
\end{figure}
...
\caption{Latency Comparison. Values follow the generated pipeline outputs and avoid over-claiming beyond the available evidence.}
\label{fig:latency}
\end{figure}
...
\caption{Rank Distribution Across Nodes. MARTHR produces more uniform distributions, indicating balanced load across the network.}
\label{fig:rank}
\end{figure}
...
\caption{Control Overhead. MARTHR maintains manageable overhead even in large grid scenarios under increasing routing stress.}
\label{fig:overhead}
\end{figure}
...
\caption{Attack Detection Performance. MARTHR successfully identifies and routes around malicious nodes across representative scenarios.}
\label{fig:detection}
\end{figure}
...
\caption{Pareto Frontier: Trust-Energy Trade-off. MARTHR explores a wider range of operating points than single-objective approaches.}
\label{fig:pareto}
\end{figure}
