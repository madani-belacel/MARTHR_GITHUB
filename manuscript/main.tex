...
Routing in mobile ad hoc networks remains challenging because wireless topologies evolve rapidly, links fail frequently, and battery resources are limited. Classical distance-vector and link-state protocols such as AODV, DSR, and OLSR~\cite{clausen2003optimized} were designed for stable MANETs and therefore struggle under high mobility and adversarial conditions. RPL (Routing Protocol for Low-power and Lossy Networks, RFC~6550) introduced the concept of an Objective Function (OF) to compute routing ranks, yet existing OFs such as MRHOF~\cite{gnaneswaran2012mrhof} and OF0 remain largely static and typically optimize a single criterion, such as hop count or expected transmission count (ETX).
...
\caption{\textbf{MARTHR Protocol Architecture.} Modular design with context fusion, trust management, score computation, and rank calculation components.}
...
\caption{\textbf{Context-Dependent Weight Adaptation.} Weights shift dynamically across safety, threat, and energy domains to emphasize the most critical objective.}
\label{fig:weights}
\end{figure}
...
\caption{\textbf{DODAG with Trust-Aware Coloring.} Higher trust values are shown in darker shades, indicating preferred routing paths.}
\label{fig:dodag}
\end{figure}
...
\caption{\textbf{MARTHR vs MRHOF Baseline.} Comparison across lossless, lossy, and attack scenarios. MARTHR achieves higher MCS and trust scores while maintaining competitive latency.}
\label{fig:baseline}
\end{figure}
...
\caption{\textbf{MCS Across Scenarios.} Mean Multi-Criteria Score from the current reproduction workflow. Values reflect present pipeline outputs.}
\label{fig:mcs}
\end{figure}
...
\caption{\textbf{Energy and MCS Summary.} MARTHR maintains high energy efficiency while achieving competitive MCS scores across representative scenarios.}
\label{fig:summary}
\end{figure}
...
\caption{\textbf{Trust Score Distributions.} Values across scenarios from the current simulation outputs, reflecting the present prototype pipeline.}
\label{fig:trust}
\end{figure}
...
\caption{\textbf{Energy Consumption Across Scenarios.} Illustrates the present generated outputs from the reproduction workflow.}
\label{fig:energy}
\end{figure}
...
\caption{\textbf{Latency Comparison.} Values follow the generated pipeline outputs and avoid over-claiming beyond the available evidence.}
\label{fig:latency}
\end{figure}
...
\caption{\textbf{Rank Distribution Across Nodes.} MARTHR produces more uniform distributions, indicating balanced load across the network.}
\label{fig:rank}
\end{figure}
...
\caption{\textbf{Control Overhead.} MARTHR maintains manageable overhead even in large grid scenarios under increasing routing stress.}
\label{fig:overhead}
\end{figure}
...
\caption{\textbf{Attack Detection Performance.} MARTHR successfully identifies and routes around malicious nodes across representative scenarios.}
\label{fig:detection}
\end{figure}
...
\caption{\textbf{Pareto Frontier: Trust-Energy Trade-off.} MARTHR explores a wider range of operating points than single-objective approaches.}
\label{fig:pareto}
\end{figure}
...
\caption{\textbf{Scenario-Proxied Component-Removal Study.} Each variant substitutes a different scenario configuration to approximate the effect of removing one lever; these are not controlled ablations on the same topology.}
\label{fig:ablation}
\end{figure}

\input{tables/ablation_table.tex}

Table~\ref{tab:ablation} summarizes scenario-proxied component-removal outputs produced by the reproducible pipeline. Because each variant substitutes a different scenario, the values should not be interpreted as a controlled ablation study.
...
The authors acknowledge the support of the University of Abdelhamid Ibn Badis, Mostaganem, Algeria.
